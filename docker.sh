...
echo_menu() {
    ...
    echo -e "${GREEN}5) Show disk usage by container name${NC}"
    echo -e "${YELLOW}6) Start all containers${NC}"
    echo -e "${YELLOW}7) Stop all running containers${NC}"
    echo -e "${YELLOW}8) Restart all running containers${NC}"
    echo -e "${RED}9) Remove all stopped containers${NC}"
    echo -e "${RED}10) Remove all unused Docker volumes${NC}"
    echo -e "${RED}11) Clean unused Docker images${NC}"
    echo -e "${RED}12) Prune entire Docker system${NC}"
    echo -e "${RED}13) Clear Docker build cache${NC}"
    echo -e "${YELLOW}14) Configure SELinux & restart Nginx${NC}"
    echo -e "${YELLOW}15) Update this script${NC}"
...
while true; do
    ...
    case "$choice" in
        ...
        4)
            ...
            ;;
        5)
            echo -e "${GREEN}Showing disk usage by container name...${NC}"
